      ...
      <div class="overflow-y-auto max-h-[calc(70vh-120px)]">
        ...
        <div class="px-4 py-3">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ t('space.pages') }}</span>
            <button
          ...
          <div v-if="!pages || pages.length === 0" class="py-6 text-center">
            <p class="text-sm text-gray-400 dark:text-slate-500">{{ t('space.noPages') }}</p>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div class="px-3 py-4" :class="sidebarCollapsed ? 'flex justify-center' : ''">
        <router-link v-if="!sidebarCollapsed" to="/" class="group flex items-center gap-3 select-none">
          ...
          <div class="flex flex-col">
            ...
            <span class="text-[10px] text-gray-400 dark:text-slate-500 font-medium tracking-wider uppercase mt-0.5">{{ t('space.bookmarkManager') }}</span>
          </div>
        </router-link>
      ...
      <div class="flex-1 overflow-y-auto" :class="sidebarCollapsed ? 'px-2' : 'px-3'">
        ...
        <div v-if="!pages || pages.length === 0" class="py-8 text-center" :class="sidebarCollapsed ? '' : 'px-3'">
          <p class="text-sm text-gray-400 dark:text-slate-500">{{ sidebarCollapsed ? '-' : t('space.noPages') }}</p>
        </div>
      </div>
      ...
      <button
        ...
        :title="sidebarCollapsed ? t('space.expandSidebar') : t('space.collapseSidebar')"
      >
